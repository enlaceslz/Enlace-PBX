        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 font-sans">
              {extensions.map((ext) => (
                <tr key={ext.id} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="py-3.5 px-4">
                    <div className="flex flex-wrap gap-1">
                      {ext.codecs.map((codec) => (
                        <span
                          key={codec}
                          className="px-1.5 py-0.2 rounded bg-slate-800 text-[10px] font-mono text-slate-300"
                        >
                          {codec}
                        </span>
                      ))}
                  ...
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <span className="capitalize text-slate-200 text-[11px] font-medium">
                        {ext.status === 'online'
                          ? 'Online'
                          : ext.status === 'busy'
                          ? 'Ocupado'
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="px-6 py-4 bg-slate-950/90 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-slate-100">Criar Novo Ramal PJSIP</h3>
