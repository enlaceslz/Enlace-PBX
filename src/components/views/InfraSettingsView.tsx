...
export const InfraSettingsView: React.FC<InfraSettingsViewProps> = ({ onOpenWebphone, onNavigate }) => {
      const [activeTab, setActiveTab] = useState<'overview' | 'network' | 'ssl' | 'services' | 'update'>('overview');
    ...
    <div className="max-w-[1400px] mx-auto space-y-8 pb-16">
      ...
      <div className="flex border-b border-slate-200 gap-2 overflow-x-auto pb-px">
        ...
        >
          ...
          <span className="ml-1.5 px-2 py-0.5 text-[10px] rounded-full bg-indigo-100 text-indigo-700 font-bold">
            GitHub
          </span>
        </button>
      </div>

      {/* TAB 5: ATUALIZAÇÃO DO SISTEMA */}
      {activeTab === 'update' && (
        <div className="space-y-6 max-w-4xl">
          <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-500 via-blue-500 to-sky-500" />
            <div className="flex items-start gap-6">
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 flex items-center justify-center shrink-0 border border-indigo-100 shadow-sm">
                <RefreshCw className={`w-8 h-8 text-indigo-600 ${updatingSystem ? 'animate-spin' : ''}`} />
              </div>
              <div className="space-y-4">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight">Atualização via GitHub (Open Source)</h3>
                  <p className="text-slate-500 mt-2 leading-relaxed">
                    Você está executando a versão open source do <strong>Enlace-PBX OSS</strong>. Ao clicar no botão abaixo, a plataforma fará um fetch automático das últimas atualizações do repositório oficial no GitHub e recompilará o projeto.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-700">Procedimento de Atualização:</span>
                  </div>
                  <ul className="text-xs text-slate-600 space-y-2 list-disc list-inside ml-1">
                    <li>Sincronização com branch principal (<code>git pull</code>)</li>
                    <li>Instalação de novas dependências (<code>npm install</code>)</li>
                    <li>Recompilação de Frontend e Backend (<code>npm run build</code>)</li>
                    <li>Reinício do Serviço PM2</li>
                  </ul>
                </div>
                ...
                <div className="pt-4">
                  <button
                    ...
                    className="flex items-center gap-3 px-6 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-slate-900/20"
                  >
                    {updatingSystem ? (
                      <>
                  ...
                  <p className="text-[11px] text-slate-400 mt-3 flex items-center gap-1.5">
                    ...
                    Aviso: O processo pode levar alguns segundos e derrubará chamadas WebRTC em andamento.
                  </p>
                </div>
              </div>
            </div>
