...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="h-16 bg-slate-50 border-b border-slate-100 px-4 md:px-6 flex items-center justify-between z-30 sticky top-0">
      {/* Brand & Slogan */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider bg-sky-950/80 text-blue-700 border border-sky-700/60 px-2 py-0.5 rounded-full">
                Asterisk 20 + Gemini
              </span>
            </div>
        ...
        <div className="hidden lg:flex items-center gap-2 pl-4 border-l border-slate-200">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-xs text-slate-600 font-mono">
          ...
          {activeCallsCount > 0 && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-sky-950/60 border border-sky-800/80 text-xs text-blue-700 font-semibold animate-pulse">
      ...
      <div className="flex items-center gap-3">
        ...
        {tenants.length > 0 && (
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-1.5 w-64 bg-white border border-slate-200 rounded-xl shadow-xl py-1.5 hidden group-hover:block z-50">
              <div className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200">
              ...
              {tenants.map((t) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs flex flex-col hover:bg-slate-100 transition ${
                    t.id === currentTenant?.id ? 'bg-sky-950/40 text-blue-700 font-medium' : 'text-slate-600'
                  }`}
                >
        ...
        <div className="flex items-center gap-2 pl-3 border-l border-slate-200">
          <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-300 flex items-center justify-center font-bold text-xs text-blue-600">
