...
export class QualityAuditRepository {
  ...
  public static async listAll(): Promise<QualityAudit[]> {
    try {
      const res = await postgresClient.query(
        'SELECT * FROM quality_audits ORDER BY created_at DESC LIMIT 100'
      );
      return res.rows.map(row => ({
        id: row.id,
        tenantId: row.tenant_id,
  ...
  public static async save(audit: QualityAudit): Promise<QualityAudit> {
    try {
      ...
      console.error('[QualityAuditRepository.save] Erro no PostgreSQL:', err?.message || err);
      throw err;
    }
  }
}
