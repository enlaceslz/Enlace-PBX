...
export class QueueRepository {
  public static async listByTenant(tenantId: string): Promise<Queue[]> {
    if (postgresClient.isConnected()) {
      try {
        ...
        console.error('[QueueRepository] Erro ao listar filas no PostgreSQL:', err.message);
      }
    }
  ...
  public static async findById(id: string, tenantId?: string): Promise<Queue | null> {
    if (postgresClient.isConnected()) {
      try {
        ...
        console.error('[QueueRepository] Erro ao buscar fila por ID:', err.message);
      }
    }
  ...
  public static async save(queue: Queue): Promise<Queue> {
    if (postgresClient.isConnected()) {
      try {
        ...
        console.error('[QueueRepository] Erro ao salvar fila no PostgreSQL:', err.message);
      }
    }
  ...
  public static async delete(id: string, tenantId: string): Promise<boolean> {
    if (postgresClient.isConnected()) {
      try {
        await postgresClient.query(
          'DELETE FROM queues WHERE id = $1 AND tenant_id = $2',
          [id, tenantId]
        );
      } catch (err: any) {
        console.error('[QueueRepository] Erro ao deletar fila no PostgreSQL:', err.message);
      }
    }
