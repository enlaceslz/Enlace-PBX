...
export class DidRepository {
  ...
  public static async findById(tenantId: string, id: string): Promise<Did | null> {
    const res = await postgresClient.query(
      'SELECT * FROM dids WHERE tenant_id = $1 AND id = $2',
      [tenantId, id]
    );
    if (res.rows.length > 0) {
      const row = res.rows[0];
  ...
  public static async delete(tenantId: string, id: string): Promise<boolean> {
    const res = await postgresClient.query(
      'DELETE FROM dids WHERE tenant_id = $1 AND id = $2',
      [tenantId, id]
    );
